...
from utils import clean_name, clean_date
...
def write_player_list(player_list):
    players_df = pd.DataFrame.from_records(player_list)
    players_df.to_csv("../dataset/tm_players.csv", index=False)
        ...
        if market_value:
            ...
            if all((name, birth_data, nationality)):
                player = {
                    ...
                    "birth_data": clean_date(birth_data.text),
                    "nationality": clean_name(nationality.text)
                }
...
def main():
    ...
    global base_url
    try:
        if os.path.exists("../datasets/tm_players.csv"):
            players_df = pd.read_csv("../datasets/tm_players.csv")
